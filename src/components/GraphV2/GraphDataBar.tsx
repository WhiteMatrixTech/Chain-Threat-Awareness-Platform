...
export const GraphDataBar = (props: IGraphDataBarProps) => {
  ...
  const handleSourceNode = useCallback(
    async () => {
  ...
  const handleTargetNode = useCallback(
    async () => {
      if (
  ...
  const handleDeleteNode = useCallback(
    ...
    [graphData.edges, graphData.nodes, changeData, changeFocusedId, focusedId]
  );
  ...
  return (
    <div
      className={cn(
        ...
        "flex absolute bottom-[10%] left-0 right-0 justify-between gap-x-4 rounded-3xl bg-[#efefef] px-10 py-3"
      )}
    >
      {tools.map(tool =>
