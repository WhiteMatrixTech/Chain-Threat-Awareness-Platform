...
export function Explorer() {
  ...
  const generateData = (data: any) => {
    ...
    const files = data.filter((item: any) => item.dir === false);
    for (const project of projects) {
      ...
      const children = targetItems.map((item: any) => {
        return {
          id: item.name,
          parentId: project.name,
          type: ExplorerItemType.FILE,
          name: item.name,
          content: ''
        };
      });
      newExplorerList.push(newProject, ...children);
    }
    return newExplorerList;
  };
  ...
  return (
    <Dropdown overlay={menu} trigger={['contextMenu']}>
      <div className="flex items-center overflow-hidden text-ellipsis whitespace-nowrap">
        {nodeData.data.name.split('/').slice(-1)}
      </div>
    </Dropdown>
  );
}
